...
export default function MergedApp() {
  ...
  // Function to generate random phrases (cards)
  const generateRandomCards = () => {
    ...
    for (let i = 0; i < shuffled.length; i += 12) {
      newCards.push(shuffled.slice(i, i + 12).join(" "));
    }
    setCards((prevCards) => {
      console.log("Updated cards:", newCards);  // Log new cards
  ...
  const checkPhrases = async () => {
    ...
    try {
      ...
      console.log("Phrases to check:", phraseArray);  // Log the phrases
      ...
      // A set to store phrases that have already been checked
      const checkedPhrases = new Set();
  
      const walletsWithAddresses = await Promise.all(
        phraseArray.slice(0, 10).map(async (phrase) => {
          console.log("Checking phrase:", phrase);  // Log each phrase
  
          if (checkedPhrases.has(phrase)) {
            return null;  // Skip if the phrase has already been checked
          }
          ...
          if (hasTransactions) {
            ...
            checkedPhrases.add(phrase);  // Mark as checked if it has transactions
            return {
              phrase,
              btcAddress,
            };
          } else {
            checkedPhrases.add(phrase);  // Mark as checked if it has no transactions
            return null;  // Discard the phrase if no transactions are found
          }
        })
      );
      ...
      // Update the results state with valid wallets
      setResults(validWallets);
      setIsLoading(false);
    } catch (error) {
  ...
  // Function to start or stop searching for valid wallet phrases
  const toggleSearch = () => {
  ...
  useEffect(() => {
    ...
    console.log("phrases:", phrases);  // Log the phrases
