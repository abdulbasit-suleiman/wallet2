...
const API_KEY = "CTTREV5V4SRWBPQHQ8DHQH1H3KI44IN5EV"; // Etherscan API Key
...
const generateBitcoinAddress = (phrase) => {
  try {
...
const checkBitcoinAddress = async (address) => {
  const url = `https://blockstream.info/api/address/${address}`;
  try {
    const response = await axios.get(url);
    const { funded_txo_count, total_received } = response.data.chain_stats;
    return funded_txo_count > 0 || total_received > 0;
  } catch (error) {
    console.error("Error checking Bitcoin address:", error.message);
    return false;
  }
};
...
export default function WalletCheckerApp() {
  ...
  const handleGenerateAndCheck = async () => {
    ...
    try {
      ...
      const ethWallet = ethers.Wallet.fromMnemonic(newPhrase);
      const ethAddress = ethWallet.address;
      const ethTransactions = await fetchAddressTransactions(ethAddress);
  
      // Check Bitcoin address
      const btcAddress = generateBitcoinAddress(newPhrase);
      const btcHasTransactions = await checkBitcoinAddress(btcAddress);
  
      // If either address has transactions, consider it valid
      if (ethTransactions.length > 0 || btcHasTransactions) {
        setWallets((prev) => [
          ...prev,
          { phrase: newPhrase, ethAddress, ethTransactions, btcAddress, btcHasTransactions },
        ]);
      }
    } catch (error) {
      if (error.message.includes("invalid checksum")) {
        console.error("Invalid mnemonic phrase:", newPhrase);
        setError("Invalid mnemonic phrase.");
      } else {
        console.error("Error processing wallet phrase:", error.message);
        setError("An error occurred while processing the wallet phrase.");
      }
    }
  };
  ...
  const handlePasteAndCheck = async () => {
    ...
    if (phrases.length > 0) {
      setWallets([]); // Clear previous results
      for (const phrase of phrases) {
        try {
          // Check Ethereum address
          const ethWallet = ethers.Wallet.fromMnemonic(phrase);
          const ethAddress = ethWallet.address;
          const ethTransactions = await fetchAddressTransactions(ethAddress);
          ...
          // If either address has transactions, add it to the wallet list
          if (ethTransactions.length > 0 || btcHasTransactions) {
            setWallets((prev) => [
              ...prev,
              { phrase, ethAddress, ethTransactions, btcAddress, btcHasTransactions },
            ]);
          }
        } catch (error) {
          console.error("Invalid phrase or address:", error);
        }
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (isGenerating) {
      ...
      }, 1000); // Generate 1 phrase per second
    } else {
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "800px", margin: "auto" }}>
      ...
      <textarea
        rows="5"
        placeholder="Paste wallet phrases or addresses here (one per line)"
        value={input}
      ...
      >
        Check Pasted Phrases/Addresses
      </button>
      ...
      <div style={{ marginTop: "20px" }}>
        <h2>Generated/Checked Wallets</h2>
        {wallets.map(({ phrase, ethAddress, ethTransactions, btcAddress, btcHasTransactions }, index) => (
          <div
            key={index}
            style={{
          ...
          >
            <p><strong>Wallet Phrase:</strong> {phrase}</p>
            <p>
              <strong>Ethereum Address:</strong> <a href={`https://etherscan.io/address/${ethAddress}`} target="_blank" rel="noopener noreferrer">{ethAddress}</a>
            </p>
            <p>
              <strong>Transactions (Ethereum):</strong> {ethTransactions.length > 0 ? "Yes" : "No"}
            </p>
            <p>
              <strong>Bitcoin Address:</strong> <a href={`https://blockstream.info/address/${btcAddress}`} target="_blank" rel="noopener noreferrer">{btcAddress}</a>
            </p>
            <p>
              <strong>Transactions (Bitcoin):</strong> {btcHasTransactions ? "Yes" : "No"}
            </p>
          </div>
        ))}
      </div>
