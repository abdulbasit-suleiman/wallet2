...
import { generatePhrases } from '../utils';  // Function to generate shuffled phrases
...
export default function BitcoinIndex() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      for (const phrase of phrases) {
        try {
          // Generate Bitcoin address from the phrase
          const address = generateBitcoinAddress(phrase);

          // Check if the generated address has any transactions or funds
          const hasTransactions = await checkBitcoinAddress(address);

          // If valid address with transactions or funds found, display it
          if (hasTransactions) {
            const phraseStatus = { phrase, address, valid: true };

            // Update checked phrases and result state
            setCheckedPhrases((prev) => [...prev, phraseStatus]);
            setResult({ phrase, address, hasTransactions: true });

            break;  // Stop after finding the first valid address
          } else {
            // If no transaction or funds, still store the phrase with the address
  ...
  const handleCustomAddressCheck = async () => {
    ...
    try {
      // Check if custom address is a valid Bitcoin address
      if (bitcoin.address.isValid(customAddress)) {
        ...
        setCustomResult("Invalid Bitcoin address format.");
      }
    } catch (error) {
          ...
          <ul style={{ listStyleType: 'none', padding: 0 }}>
            {checkedPhrases.map((phraseStatus, index) => (
              <li key={index} style={{ marginBottom: '5px' }}>
                {phraseStatus.phrase} - {phraseStatus.valid ? <span style={{ color: 'green' }}>Valid</span> : <span style={{ color: 'red' }}>Invalid</span>}
              </li>
            ))}
          </ul>
        </div>
          ...
          <ul style={{ listStyleType: 'none', padding: 0 }}>
            {checkedPhrases.map((phraseStatus, index) => (
              <li key={index} style={{ marginBottom: '10px' }}>
                <p><strong>Phrase:</strong> {phraseStatus.phrase}</p>
                <p><strong>Bitcoin Address:</strong> {phraseStatus.address}</p>
                <p>{phraseStatus.valid ? <span style={{ color: 'green' }}>Valid Address with Transactions</span> : <span style={{ color: 'red' }}>No Transactions or Funds</span>}</p>
              </li>
            ))}
          </ul>
        </div>
